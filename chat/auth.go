...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "login":
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("Error when trying to GetBeginAuth URL for %s:%s", provider, err), http.StatusInternalServerError)
			return
		}
	...
	default:
		...
		fmt.Fprintf(w, "Auth action %s not supported", r.URL.Path)
		fmt.Fprintf(w, "Auth action %s not supported", action)
	}
}

// MustAuth used for chat
